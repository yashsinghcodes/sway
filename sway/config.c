...
static char *expand_line(const char *block, const char *line, bool add_brace) {
  int size =
      (block ? strlen(block) + 1 : 0) + strlen(line) + (add_brace ? 2 : 0) + 1;
...
bool read_config(FILE *file, struct sway_config *config,
  ...
  while ((nread = getline_with_cont(&line, &line_size, file, &nlines)) != -1) {
    ...
    case CMD_FAILURE:
    case CMD_INVALID:
      sway_log(SWAY_ERROR, "Error on line %i '%s': %s (%s)", line_number, line,
               res->error, config->current_config_path);
      if (!config->validating) {
